...
void main()
{
   ...
   for(int i = 0; i < lightCounter_size; i++)
   {
      ...
      vec3 radiance = light_D.Color_And_Intensity.xyz * attenuation;

      float NDF = calc_DistributionGGX(N, H, gSpecular);
      float G = calc_GeometrySmith(N, V, L, gSpecular);
    ...
    vec3 color = gDiffuse + Lo;

    color = color / (color + vec3(1.0));
    color = pow(color, vec3(1.0 / 2.2));

   // FragColor = vec4(color, 1.0);
    FragColor = vec4(0.3,0.3,0.3, 1.0);   /////RESOLVE THE SCREEN IN BEAUTY PASS
}